        ...
        if (item.Nombre > 0) {
            if (item.Data == null) {
                ...
                if (itemid == 'CarteCr') {
                    itemid = 'cartecredit';
                    var IData = item.Data
                } else if (itemid == 'WEAPON_') {
                    var IData = 'Munition : ' + item.Data.Munition;
                    itemid = item.ID;
                } else if (itemid == 'Telepho') {
                    itemid = 'telephone';
                    var IData = "Numéro : " + item.Data.Num
                } else if (itemid == 'CleVoit') {
                    var IData = item.Data
                } else if (itemid == 'patedec') {
                    itemid = 'patedecocaine';
                    var IData = "Pureté : " + item.Data + "%"
                    ...
                    if (item.Data.Type == 'Pantalon') {
                        itemid = 'vet_bas';
                    }
                    if (item.Data.Type == 'Chaussure') {
                        itemid = 'vet_chaussure';
                    }
                    if (item.Data.Type == 'Accessoire') {
                        itemid = 'vet_accessoire';
                    }
...
function secondInventorySetup(items) {
    ...
    $.each(items, function(index, item) {
        ...
        if (item.Nombre > 0) {
            ...
            if (item.Data == null) {
                $("#otherInventory").append('<div class="slot"><div id="itemOther-' + index + '" class="item" style = "background-image: url(\'img/items/' + item.ID + '.png\')">' +
                '<div class="item-count">' + count + '</div> <div class="item-name">' + item.Nom + '</div> </div ><div class="item-name-bg"></div></div>');
            } else {
                var itemid = item.ID.substr(0, 7);
                if (itemid == 'CarteCr') {
                ...
                $("#otherInventory").append('<div class="slot"><div id="itemOther-' + index + '" class="item" style = "background-image: url(\'img/items/' + itemid + '.png\')">' +
                '<div class="item-count">' + count + '</div> <div class="item-name">' + item.Nom + '</div> <div class="item-data">' + '(' + IData + ')' + '</div> </div ><div class="item-name-bg"></div></div>');
            }
            $('#itemOther-' + index).data('item', item);
            $('#itemOther-' + index).data('inventory', "second");
