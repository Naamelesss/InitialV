...
mainMenu = NativeUI.CreateMenu("","Armes & Accessoires", nil, nil, "commonmenu", "ammunation")
_menuPool:Add(mainMenu)

function ShopMenu(menu)
    local gunmenu = _menuPool:AddSubMenu(menu, "Armes", "", nil, "commonmenu", "ammunation")
    local accmenu = _menuPool:AddSubMenu(menu, "Accessoires", "", nil, "commonmenu", "ammunation")
        ...
        if item == Couteau then
            if HaveCouteau ~= 0 then
                if PlayerMoney >= Prix.Couteau then
                    ...
                    TriggerServerEvent('InitialCore:TransferArmeToPlayer', 'CONSOLE', GetPlayerServerId(PlayerId()), 'WEAPON_KNIFE', 1)
                    GiveWeaponToPed(GetPlayerPed(-1), -1716189206, 0, false, true)
                    Couteau:SetRightBadge(BadgeStyle.Gun)
                    Couteau:RightLabel("")
                    DisplayBoughtScaleform("WEAPON_KNIFE", "Couteau")
                else
        ...
        elseif item == CouteauCranArret then
            if HaveCouteauCranArret ~= 0 then
                if PlayerMoney >= Prix.CouteauCranArret then
                    ...
                    DisplayBoughtScaleform("WEAPON_SWITCHBLADE", "Couteau à cran d'arret")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == LampeDePoche then
            if HaveLampeDePoche ~= 0 then
                if PlayerMoney >= Prix.LampeDePoche then
                    ...
                    DisplayBoughtScaleform("WEAPON_FLASHLIGHT", "Lampe torche")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == Batte then
            if HaveBatte ~= 0 then
                if PlayerMoney >= Prix.Batte then
                    ...
                    Batte:SetRightBadge(BadgeStyle.Gun)
                    Batte:RightLabel("")
                    DisplayBoughtScaleform("WEAPON_BAT", "Batte")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == Machete then
            if HaveMachete ~= 0 then
                if PlayerMoney >= Prix.Machete then
                    ...
                    Machete:SetRightBadge(BadgeStyle.Gun)
                    Machete:RightLabel("")
                    DisplayBoughtScaleform("WEAPON_MACHETE", "Machette")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == SNSPistol then
            if HaveSNSPistol ~= 0 then
                if PlayerMoney >= Prix.SNSPistol then
                    ...
                    DisplayBoughtScaleform("WEAPON_SNSPISTOL", "Pétoire")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == Pistol then
            if HavePistol ~= 0 then
                if PlayerMoney >= Prix.Pistol then
                    ...
                    Pistol:SetRightBadge(BadgeStyle.Gun)
                    Pistol:RightLabel("")
                    DisplayBoughtScaleform("WEAPON_PISTOL", "Pistolet")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
        ...
        elseif item == CombatPistol then
            if HaveCombatPistol ~= 0 then
                if PlayerMoney >= Prix.CombatPistol then
                    ...
                    DisplayBoughtScaleform("WEAPON_COMBATPISTOL", "Pistolet de combat")
                else
                    AdvancedNotif("Ammu-Nation", "Erreur", "Pas assez d'argent.", "CHAR_WE", "INITIALV")
                end
            else
    ...
    accmenu.SubMenu.OnItemSelect = function(menu, item)
        ...
        if item == WeaLampe then
            if PlayerMoney >= Prix.Lampe then
                TriggerServerEvent('InitialCore:TransferMoney', GetPlayerServerId(PlayerId()), 'CONSOLE', Prix.Lampe)
                PlaySoundFrontend(-1, 'WEAPON_AMMO_PURCHASE', 'HUD_AMMO_SHOP_SOUNDSET', false)
...
function DisplayBoughtScaleform(weaponID, weaponName)
    ...
    PushScaleformMovieMethodParameterString(weaponName)
	PushScaleformMovieMethodParameterInt(GetHashKey(weaponID))
        ...
        for k in pairs(AmmuLocation) do
            ...
            if dist <= 2.0 then
                ...
                if IsControlJustPressed(1,51) then 
                    TriggerServerEvent('InitialCore:RecupInfoLicenseDarmes')
				end
            end
        end
    end
...
RegisterNetEvent('InitialCore:RecupInfoLicenseDarmesC')
AddEventHandler('InitialCore:RecupInfoLicenseDarmesC', function(license)
    if tobool(license) then
        mainMenu:Clear()
        ShopMenu(mainMenu)
        _menuPool:RefreshIndex()
        mainMenu:Visible(not mainMenu:Visible())
    else
       AdvancedNotif("Ammu-Nation", "License", "Je ne peut pas vous proposer d'armes sans votre license.", "CHAR_WE", "INITIALV")
    end
end)
