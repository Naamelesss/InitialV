...
function CreateInstance(type, player, hashid, data)
	instances[player] = {
		type    = type,
		host    = player,
		hashid  = hashid,
...
AddEventHandler('instance:create', function(type, hashid, data)
	...
	CreateInstance(type, source, hashid, data)
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll("SELECT * FROM instanceobject", {}, function(result)
		infoobject = result
        --for k in ipairs(result) do
			--print(k)
			--print(result[1].ObjectID)
			--print(IC.Items["IDCard#639126586"].Nom)		
		--end
		
		--for k,v in ipairs(infoobject) do
		--	print(v.ID)
		--	IC.ObjectInstance[v.ID] = {
		--		ObjectID = v.ObjectID,
		--		Coords = v.Coords
		--	}
			--print(IC.ObjectInstance[v])
		--end
		--for k in ipairs(IC.ObjectInstance) do
		--	print(v[])
		--end
		--print(IC.ObjectInstance[1])

		local instanceObjectInfo = {}

		for k in pairs (result) do
			--print(k)
			--print( json.decode(result[k].Coords).z )

			table.insert(instanceObjectInfo, {
				ID = result[k].ID,
				ObjectID = result[k].ObjectID,
				Coords = json.decode(result[k].Coords),
				Temps = result[k].Temps,
			})

			--print(instanceObjectInfo[k].Coords.z)
		end

		for k in pairs (instanceObjectInfo) do

			--print(instanceObjectInfo[k].ID)
