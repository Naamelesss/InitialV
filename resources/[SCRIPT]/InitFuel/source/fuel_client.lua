...
local ret, currentCash = StatGetInt("MP0_WALLET_BALANCE", -1)
...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		--if currentCash >= currentCost then
			--print(currentCost)
			TriggerServerEvent('InitialCore:TransferMoney', GetPlayerServerId(PlayerId()), 'CONSOLE', math.floor(currentCost + 0.5))
		--end
	end
	currentCost = 0.0
end)
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								if PlayerMoney >= 35 then
								  TriggerServerEvent('InitialCore:TransferMoney', GetPlayerServerId(PlayerId()), 'CONSOLE', 35)	
						...
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								...
								if PlayerMoney >= 100 then
									TriggerServerEvent('InitialCore:TransferMoney', GetPlayerServerId(PlayerId()), 'CONSOLE', 100)
								end
							end
						else
							if Config.UseESX then
