...
RegisterServerEvent('InitialCore:SetNumber')
AddEventHandler('InitialCore:SetNumber', function(number)
    MyNumberVar = number
end)
...
function getNumberPhone(identifier)
   ...
   -- print(MyNumberVar .. "VAR")
    return MyNumberVar
end
function getIdentifierByPhoneNumber(phone_number) 
    local result = MySQL.Sync.fetchAll("SELECT SteamID FROM phone WHERE Phone = @phone_number", {
...
function getContacts(num)
    local result = MySQL.Sync.fetchAll("SELECT * FROM phonecontacts WHERE identifier = @num", {
        ['@num'] = num
    })
    return result
end
function addContact(source, identifier, number, display, mynum)
    local sourcePlayer = tonumber(source)
    MySQL.Async.insert("INSERT INTO phonecontacts (`identifier`, `number`,`display`) VALUES(@identifier, @number, @display)", {
        ['@identifier'] = mynum,
        ['@number'] = number,
        ['@display'] = display,
    },function()
...
AddEventHandler('gcPhone:addContact', function(display, phoneNumber, mynum)
    local sourcePlayer = tonumber(source)
    local identifier = getPlayerID(source)
    addContact(sourcePlayer, identifier, phoneNumber, display, mynum)
end)
...
function getMessages(num)
    print('GETMSG')
    local result = MySQL.Sync.fetchAll("SELECT * FROM phonemessage LEFT JOIN phone ON Phone = @number WHERE receiver = Phone", {
         ['@number'] = num
    })
...
function _internalAddMessage(transmitter, receiver, message, owner)
    local Query = "INSERT INTO phonemessage (`transmitter`, `receiver`,`message`, `isRead`,`owner`) VALUES(@transmitter, @receiver, @message, @isRead, @owner);"
    local Query2 = 'SELECT * from phonemessage WHERE `ID` = @id;'
	local Parameters = {
...
function addMessage(source, identifier, myphonenumer, phone_number, message)
    local sourcePlayer = tonumber(source)
    local otherIdentifier = getIdentifierByPhoneNumber(phone_number)
    --local myPhone = getNumberPhone(identifier)
    local myPhone = myphonenumer
    print(myPhone .. " My Phone")
    print(phone_number .. " Other Phone")
...
function setReadMessageNumber(identifier, num, mynum)
    local mePhoneNumber = getNumberPhone(identifier)
    MySQL.Sync.execute("UPDATE phonemessage SET isRead = 1 WHERE receiver = @receiver AND transmitter = @transmitter", { 
        ['@receiver'] = mynum,
        ['@transmitter'] = num
    })
end
...
AddEventHandler('gcPhone:sendMessage', function(myphonenumer, phoneNumber, message)
    print(phoneNumber .. 'OTHERPHONE')
    local sourcePlayer = tonumber(source)
    local identifier = getPlayerID(source)
    addMessage(sourcePlayer, identifier, myphonenumer, phoneNumber, message)
end)
...
AddEventHandler('gcPhone:setReadMessageNumber', function(num, mynum)
    local identifier = getPlayerID(source)
    setReadMessageNumber(identifier, num, mynum)
end)
...
function getHistoriqueCall (num)
    local result = MySQL.Sync.fetchAll("SELECT * FROM phonecalls WHERE owner = @num ORDER BY time DESC LIMIT 120", {
        ['@num'] = num
    })
    return result
end
...
function saveAppels (appelInfo)
  --  print(appelInfo.transmitter_num .. 'tra')
   -- print(appelInfo.receiver_num .. "rec")
    if appelInfo.extraData == nil or appelInfo.extraData.useNumber == nil then
        MySQL.Async.insert("INSERT INTO phonecalls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)", {
...
RegisterServerEvent('NPhone:Refresh')
AddEventHandler('NPhone:Refresh', function(num)
    print('refresh' .. num)
    local srcPlayer = tonumber(source)
   -- TriggerClientEvent("gcPhone:myPhoneNumber", source, myPhoneNumber)
    TriggerClientEvent("gcPhone:contactList", source, getContacts(num))
    TriggerClientEvent("gcPhone:allMessage", srcPlayer, getMessages(num))
end)
